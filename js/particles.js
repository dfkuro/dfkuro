...
function Circle() {
	...
	this.reset = function() {
		...
		this.dx = (Math.random()*this.s.xmax) * (Math.random() < .5 ? -2 : 2);
		this.dy = (Math.random()*this.s.ymax) * (Math.random() < .5 ? -2 : 2);
		this.hl = (this.s.ttl/rint)*(this.r/this.s.rmax);
		this.rt = Math.random()*this.hl;
		this.s.rt = Math.random()+1;
		this.stop = Math.random()*.2+.4;
		this.s.xdrift *= Math.random() * (Math.random() < .5 ? -3 : 3);
		this.s.ydrift *= Math.random() * (Math.random() < .5 ? -3 : 3);
	}
	...
	this.draw = function() {
		...
		g.addColorStop(this.stop, 'rgba(0,'+parseInt(Math.random()*255)+','+parseInt(Math.random()*155+50)+','+(newo*.7)+')');
